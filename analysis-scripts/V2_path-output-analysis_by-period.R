...
for(t in 1:length(water.levels)) {
  ...
  if(length(files) != 0) {
    ...
    ds.comb = data.frame(x=double(0), y = double(0), period=character(0))
    ...
    if(nrow(ds.comb) > 1) {
      for(p in unique(ds.comb$period)) {
        dat.period = ds.comb[which(ds.comb$period == p),]
        ...
        #plot(r.sub)
        #plot(costRast)
        writeRaster(r.sub, paste0(getwd(), "/routes/changing/", water, "_", p, "_routes.asc"), overwrite = T)
      }
