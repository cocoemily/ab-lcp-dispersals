  ...
  for(t in 1:length(timeperiods)) {
    ...
    if(length(files) != 0) {
      ...
      for(l in 1:list_size[1]){
        ...
        dat.clean$step = as.numeric(dat.clean$step)
        ...
        if(success == T) {
          ...
          line.out = st_sfc(st_linestring(cbind(dat.clean$x, dat.clean$y)), crs = 3857)
          
          last.point = dat.clean[nrow(dat.clean),]
          point = st_as_sf(x = last.point, coords = c("x", "y"), crs = 3857)
          
          st_write(line.out, paste0("routes/individual/RA_success/shapefiles/", start, "_", period, "_route", l, "_linestring.shp"), delete_dsn = T)
          st_write(line, paste0("routes/individual/RA_success/shapefiles/", start, "_", period, "_route", l, "_line.shp"), delete_dsn = T)
          st_write(point, paste0("routes/individual/RA_success/shapefiles/", start, "_", period, "_route", l, "_point.shp"), delete_dsn = T)
        }
      }
    }
